...
class ExpenseService:
    ...
    def get_by_project(db: Session, project_id: int) -> List[Expense]:
        """Get all expenses for a project (including general expenses)."""
        return db.query(Expense).filter(
            (Expense.project_id == project_id) | (Expense.is_general == True)
        ).all()

    @staticmethod
    def get_project_expenses(db: Session, project_id: int) -> List[Expense]:
        """Get only project-specific expenses."""
        return db.query(Expense).filter(
            Expense.project_id == project_id,
            Expense.is_general == False
        ).all()

    @staticmethod
    def get_general_expenses(db: Session) -> List[Expense]:
        """Get all general expenses."""
        return db.query(Expense).filter(Expense.is_general == True).all()
    ...
    def get_total_expenses(db: Session, project_id: int) -> float:
        """Get total expenses for a project (including general)."""
